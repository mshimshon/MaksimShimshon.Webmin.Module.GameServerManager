...
public class ExecRestartServerHandler : IRequestHandler<ExecRestartServerCommand, ServerInfoEntity?>
{
    ...
    public async Task<ServerInfoEntity?> Handle(ExecRestartServerCommand request, CancellationToken cancellationToken)
    {
        try
        {
            return await _lifecycleServices.ServerRestartAsync();
        }
